...
@configclass
class Args(ExperimentArgs, Args):
    pass
...
def main(args):
    ...
    if args.agent_type == "MLPTD3Agent":
        actor_class = MLPTD3Actor
        critic_class = MLPTD3Critic
    elif args.agent_type == "CNNTD3Agent":
        actor_class = CNNTD3Actor
        critic_class = CNNTD3Critic
    else:
    ...
    q_optimizer = optim.Adam(list(critic.parameters()), lr=args.q_lr)
    actor_optimizer = optim.Adam(list(actor.parameters()), lr=args.actor_lr)
    q_scheduler = optim.lr_scheduler.CosineAnnealingLR(
        q_optimizer, T_max=args.total_timesteps, eta_min=args.q_lr_min
    )
    actor_scheduler = optim.lr_scheduler.CosineAnnealingLR(
        actor_optimizer, T_max=args.total_timesteps, eta_min=args.actor_lr_min
    )
